...
package ivorius.psychedelicraft.internal.raytracing;
...
public class IvRaytracer {
    ...
    public static List<IvRaytracedIntersection> getIntersections(List<IvRaytraceableObject> objects, double x, double y,
        double z, double xDir, double yDir, double zDir) {
    ...
    public static IvRaytracedIntersection findFirstIntersection(List<IvRaytracedIntersection> intersections, double x,
        double y, double z, double xDir, double yDir, double zDir) {
        IvRaytracedIntersection firstPoint = null;
        double firstDistanceSQ = -1;

        for (IvRaytracedIntersection point : intersections) {
            double pointDistSQ = (x - point.getX()) * (x - point.getX()) + (y - point.getY()) * (y - point.getY())
                + (z - point.getZ()) * (z - point.getZ());
            boolean forwards = (point.getX() - x > 0) == (xDir > 0) && (point.getY() - y > 0) == (yDir > 0)
                && (point.getZ() - z > 0) == (zDir > 0);
    ...
    public static IvRaytracedIntersection getFirstIntersection(List<IvRaytraceableObject> objects, double x, double y,
        double z, double xDir, double yDir, double zDir) {
