...
public class IvRaytraceableAxisAlignedSurface extends IvRaytraceableObject {
    ...
    public IvRaytraceableAxisAlignedSurface(Object userInfo, Object hitInfo, double y, double z, double width,
    ...
    public void addRaytracedIntersectionsForLineToList(List<IvRaytracedIntersection> list, double x, double y, double z,
        ...
        if (!cantHit) {
            ...
            if (withinBounds(hitPoint[0], this.x, this.x + this.width)
                && withinBounds(hitPoint[1], this.y, this.y + this.height)
                && withinBounds(hitPoint[2], this.z, this.z + this.depth)) {
    ...
    private void checkValid() {
        ...
        if (lengths0 != 1) {
            throw new ArithmeticException(
                "Axis aligned surface must have exactly one length that is zero. (But has: " + lengths0 + ")");
        }
    }
