...
public class IvBezierPath3D {
    ...
    public void buildDistances() {
        ...
        for (IvBezierPoint3D bezierPoint : bezierPoints) {
            if (previousPoint != null) {
                ...
                for (int i = 0; i < samples; i++) {
                    ...
                    double[] position = IvVecMathHelper.cubicMix(
                        previousPoint.position,
                        bezierFrom,
                        bezierTo,
                        bezierPoint.position,
                        (double) i / (double) samples);
                    double[] position1 = IvVecMathHelper.cubicMix(
                        previousPoint.position,
                        bezierFrom,
                        bezierTo,
                        bezierPoint.position,
                        (double) (i + 1) / (double) samples);
        ...
        cachedProgresses.addAll(
            cachedDistances.stream()
                .map(d -> d / cachedFullDistance)
    ...
    private IvBezierPoint3DCachedStep getCachedStep(int leftIndex, int rightIndex, double leftProgress,
        double rightProgress, double innerProgress) {
        return new IvBezierPoint3DCachedStep(
            bezierPoints.get(leftIndex),
            leftIndex,
            bezierPoints.get(rightIndex),
            rightIndex,
            leftProgress,
            rightProgress,
            innerProgress);
    ...
    public double[] getPVector(IvBezierPoint3DCachedStep cachedStep, double stepSize) {
        ...
        //
        // double[] pVector = IvMathHelper.crossProduct(IvMathHelper.mix(motion1, motionNext1, cachedStep.progress),
        // IvMathHelper.mix(motion2, motionNext2, cachedStep.progress));
