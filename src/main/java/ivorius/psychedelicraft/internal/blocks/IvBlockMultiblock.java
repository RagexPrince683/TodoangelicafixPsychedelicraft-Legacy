...
package ivorius.psychedelicraft.internal.blocks;
...
public abstract class IvBlockMultiblock extends BlockContainer {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int blockMeta) {
        ...
        if (tileEntity instanceof IvTileEntityMultiBlock) {
            ...
            if (!tileEntityMultiBlock.multiblockInvalid) {
                ...
                else destroyParentOf(tileEntityMultiBlock);
            }
        }
    ...
    public void destroyChildrenOf(World world, int x, int y, int z, Block block, int blockMeta,
        IvTileEntityMultiBlock tileEntityMultiBlock) {
        if (!world.isRemote) this.parentBlockDropItemContents(world, tileEntityMultiBlock, x, y, z, block, blockMeta);
    ...
    protected void triggerBlockDestroyEffects(World world, int x, int y, int z) {
        ...
        // if (world.isRemote)
        // Minecraft.getMinecraft().effectRenderer.addBlockDestroyEffects(parentTE.xCoord, parentTE.yCoord,
        // parentTE.zCoord, parentTE.getBlockType(), parentTE.getBlockMetadata());
    }
    ...
    public boolean removedByPlayer(World world, EntityPlayer player, int x, int y, int z, boolean willHarvest) {
        ...
        if (tileEntityMultiBlock != null) {
            if (!world.isRemote && willHarvest) this
                .parentBlockHarvestItem(world, tileEntityMultiBlock, x, y, z, this, world.getBlockMetadata(x, y, z));
        }
    ...
    public void parentBlockDropItemContents(World world, IvTileEntityMultiBlock tileEntity, int x, int y, int z,
        Block block, int metadata) {}

    public void parentBlockHarvestItem(World world, IvTileEntityMultiBlock tileEntity, int x, int y, int z, Block block,
        int metadata) {}
    ...
    public static boolean validateMultiblock(Block block, IBlockAccess access, int x, int y, int z) {
        ...
        if (destroy) {
            if (access instanceof World) ((World) access).setBlockToAir(x, y, z);
        }
    ...
    public static IvTileEntityMultiBlock getValidatedTotalParent(Block block, IBlockAccess access, int x, int y,
        int z) {
