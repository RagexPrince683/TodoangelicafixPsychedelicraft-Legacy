...
import ivorius.psychedelicraft.internal.math.IvVecMathHelper;
import ivorius.psychedelicraft.internal.raytracing.IvRaytraceableAxisAlignedBox;
import ivorius.psychedelicraft.internal.tools.IvNBTHelper;
...
public class IvTileEntityMultiBlock extends TileEntity {
    ...
    public void becomeChild(TileEntity parent) {
        if (parent != null) {
            this.parentCoords = new int[] { parent.xCoord - this.xCoord, parent.yCoord - this.yCoord,
    ...
    public IvTileEntityMultiBlock getParent() {
        if (parentCoords != null) {
            ...
            TileEntity parentTileEntity = getWorldObj()
                .getTileEntity(parentCoords[0], parentCoords[1], parentCoords[2]);

            if (parentTileEntity != null && this.getClass()
                .equals(parentTileEntity.getClass())) {
    ...
    public float getDistanceToCenter() {
        return (float) IvVecMathHelper
            .distance(new double[] { xCoord + 0.5, yCoord + 0.5, zCoord + 0.5 }, getActiveCenterCoords());
    }
    ...
    public IvRaytraceableAxisAlignedBox getRotatedBox(Object userInfo, double x, double y, double z, double width,
        ...
        return IvMultiBlockHelper
            .getRotatedBox(userInfo, x, y, z, width, height, depth, getDirection(), getActiveCenterCoords());
    }
